...
export default function SignupForm({ error }: { error?: string }) {
  ...
  return (
    <form action={signup} className="login-card">
      ...
      <label>I mainly work as<select name="creator_type" required defaultValue="Artist" className="dark-select"><option>Artist</option><option>Producer</option><option>Artist + Producer</option><option>Songwriter / Composer</option><option>Engineer / Technical</option><option>Manager</option><option>Videographer / Photographer</option><option>Designer / Visual Creative</option><option>Content / Media</option><option>Session Musician</option><option>Other Creative</option></select><small>Project Lead, A&amp;R and administrative permissions are assigned by the project team.</small></label>
