...
export async function updateOwnBeatMetadata(formData:FormData){
  ...
  const{error}=await admin.from("beats").update({
    title:read(formData,"title")||null,
    producer_name:read(formData,"producer_name")||null,
    bpm:read(formData,"bpm")?Number(read(formData,"bpm")):null,
    musical_key:read(formData,"musical_key")||null,
    genre_tags:read(formData,"genre_tags").split(",").map(x=>x.trim()).filter(Boolean),
    mood_tags:read(formData,"mood_tags").split(",").map(x=>x.trim()).filter(Boolean),
    description:read(formData,"description")||null,
    artist_capacity:capacity,
    updated_at:new Date().toISOString(),
  }).eq("id",beatId).eq("project_id",project.id);
...
export async function prepareR2BeatUpload(formData:FormData){
  ...
  if(!hasAnyRole(roles,["Super Admin","Admin","Project Lead","A&R","Producer"]))throw new Error("Your project role cannot upload beats.");
  if(!isR2Configured())throw new Error("Cloudflare R2 is not configured yet. Add the R2 server variables before direct uploads.");
  const name=read(formData,"file_name"),mime=read(formData,"file_type"),size=Number(read(formData,"file_size"));
...
export async function addBeat(formData:FormData){
  ...
  const storageProvider=read(formData,"storage_provider"),storageKey=read(formData,"storage_key"),playbackUrl=read(formData,"playback_url");
  if(storageProvider==="r2"&&storageKey)await assertR2ObjectAccess(storageKey);
  ...
  const{data:beat,error}=await supabase.from("beats").insert({
    ...
    musical_key:read(formData,"musical_key")||null,
    genre_tags:read(formData,"genre_tags").split(",").map(v=>v.trim()).filter(Boolean),
    mood_tags:read(formData,"mood_tags").split(",").map(v=>v.trim()).filter(Boolean),
    description:read(formData,"description")||null,
    artist_capacity:Number(read(formData,"artist_capacity"))||project.default_beat_capacity||3,
    status:"available",
    created_by:user.id,
  }).select("id").single();
  if(error){if(audioPath)await supabase.storage.from("beat-audio").remove([audioPath]);throw new Error(error.message);}
...
export async function leaveIdea(formData:FormData){
  ...
  const body=read(formData,"idea");if(!body)go("error","Write your idea first.");
  const beatId=read(formData,"beat_id");
...
export async function manageClaim(formData:FormData){
  ...
  if(!project||!hasAnyRole(roles,["Super Admin","Admin","Project Lead","A&R"]))go("error","Only project leadership can manage claims.");
  const claimId=read(formData,"claim_id"),status=read(formData,"status"),reason=read(formData,"reason");
...
export async function convertToTrack(formData:FormData){
  const{user,project,roles}=await getWorkspace();
  if(!project||!hasAnyRole(roles,["Super Admin","Admin","Project Lead","A&R"]))go("error","Only project leadership can start track development.");
  const beatId=read(formData,"beat_id"),title=read(formData,"title")||"Untitled track";
  const admin=createAdminClient();

  const{data:beat,error:beatError}=await admin.from("beats").select("id,producer_user_id").eq("id",beatId).eq("project_id",project.id).maybeSingle();
  if(beatError)go("error",beatError.message);
  if(!beat)go("error","This beat is not available in the current project.");

  // Intentionally allow the same beat to create more than one track.
  const{data:track,error}=await admin.from("tracks").insert({
    project_id:project.id,beat_id:beatId,
    track_code:`TRACK-${Date.now().toString().slice(-5)}`,
    working_title:title,status:"in_development",development_status:"in_development",created_by:user.id,
  }).select("id").single();
  if(error)go("error",error.message);

  const claims=await admin.from("beat_claims").select("artist_id").eq("beat_id",beatId).in("status",["claimed","confirmed","converted_to_track"]);
  const contributors=(claims.data||[]).map(row=>({track_id:track!.id,user_id:row.artist_id,contribution_role:"artist"}));
  const producerUserId=beat?.producer_user_id||null;
  if(producerUserId)contributors.push({track_id:track!.id,user_id:producerUserId,contribution_role:"producer"});
  if(contributors.length){
    const unique=Array.from(new Map(contributors.map((row:any)=>[`${row.user_id}:${row.contribution_role}`,row])).values());
    await admin.from("track_contributors").insert(unique);
  }

  // Do NOT convert/remove claims and do NOT mark the beat as used.
  // It remains a reusable library asset and its development slots stay visible.
  await admin.from("activity_log").insert({project_id:project.id,user_id:user.id,action:`started ${title} from reusable beat`,entity_type:"track",entity_id:track!.id});
  await admin.from("platform_events").insert({user_id:user.id,project_id:project.id,event_name:"track_started_from_reusable_beat",category:"music",entity_type:"track",entity_id:track!.id,metadata:{beat_id:beatId}});
  revalidatePath("/beats");revalidatePath("/tracks");revalidatePath("/workspace");
  go("message","Track development started. The beat remains available for another song or version.");
}
