...
export async function inviteExistingUser(formData: FormData) {
  ...
  const { data: profile } = await admin.from("profiles").select("id,full_name,stage_name,email").eq("email", email).maybeSingle();
  if (!profile) throw new Error("That person needs a FACKTS Music account before you can invite them in-app.");
