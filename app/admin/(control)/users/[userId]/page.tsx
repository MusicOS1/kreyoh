...
import { addUsersToManagedProject, removeManagedMember, restoreManagedMember, updateManagedCreatorProfile } from "../../actions";
...
export default async function AdminUserProfile({ params }: { params: Promise<{ userId: string }> }) {
  ...
  const creatorTypes=["Artist","Producer","Songwriter / Composer","Engineer / Technical","Manager","Videographer / Photographer","Designer / Visual Creative","Content / Media","Session Musician","Other Creative"];
  ...
  return <>
    ...
    <section className="control-grid">
      <article className="control-panel"><header><div><span className="control-eyebrow">CREATOR PROFILE</span><h2>Edit identity and EPK</h2></div></header><form action={updateManagedCreatorProfile} className="control-profile-add"><input type="hidden" name="user_id" value={user.id}/><input name="full_name" defaultValue={user.full_name||""} placeholder="Full name" required/><input name="stage_name" defaultValue={user.stage_name||""} placeholder="Stage name"/><input name="nickname" defaultValue={user.nickname||""} placeholder="Nickname"/><textarea name="bio" defaultValue={user.bio||""} placeholder="Short bio"/><input name="location" defaultValue={user.location||""} placeholder="Location"/><div>{creatorTypes.map(type=><label key={type}><input type="checkbox" name="creator_types" value={type} defaultChecked={(user.creator_types||[]).includes(type)}/>{type}</label>)}</div><input type="url" name="social_url" defaultValue={user.social_links?.primary||""} placeholder="Primary social link"/><input type="url" name="streaming_url" defaultValue={user.streaming_links?.primary||""} placeholder="Primary streaming link"/>{Array.from({length:5},(_,index)=>{const song=user.top_songs?.[index]||{};return <div key={index}><input name={`song_${index+1}_title`} defaultValue={song.title||""} placeholder={`Top song ${index+1}`}/><input type="url" name={`song_${index+1}_url`} defaultValue={song.url||""} placeholder="Song link"/></div>})}<button>Save creator profile</button></form></article>
