...
export default async function AdminUserProfile({ params }: { params: Promise<{ userId: string }> }) {
  ...
  const [{ data: user }, { data: projects = [] }, { data: roles = [] }, { data: events = [] }, { count: uploads }, { count: claims }, { count: tasks }] = await Promise.all([
    ...
    admin.from("project_tasks").select("id", { count: "exact", head: true }).eq("assigned_to", userId).eq("status", "done"),
  ]);
  if (!projects || !roles || !events) throw new Error("Control Room user details could not be loaded.");
  if (!user) notFound();
  const memberships = (user.project_members || []).filter((member: any) => member.status === "active");
  return <>
    <section className="control-page-hero users"><span className="control-eyebrow">ADMIN USER PROFILE</span><h1>{user.stage_name || user.full_name || "Creator"}</h1><p>{user.email} · {(user.creator_types || []).join(" · ") || "Creator"}</p></section>
    <section className="control-metrics"><article><span>Projects</span><strong>{memberships.length}</strong><small>Active memberships</small></article><article><span>Beats uploaded</span><strong>{uploads || 0}</strong><small>Creative activity</small></article><article><span>Claims</span><strong>{claims || 0}</strong><small>Development slots</small></article><article><span>Tasks done</span><strong>{tasks || 0}</strong><small>Recorded completions</small></article><article><span>Last login</span><strong className="control-date-value">{user.last_login_at ? new Date(user.last_login_at).toLocaleDateString("en-KE") : "—"}</strong><small>{user.last_login_at ? new Date(user.last_login_at).toLocaleTimeString("en-KE") : "Not recorded"}</small></article><article><span>Status</span><strong className="control-date-value">{user.account_status || "active"}</strong><small>{user.global_role || "creator"}</small></article></section>
    <section className="control-grid"><article className="control-panel"><header><div><span className="control-eyebrow">ACCESS</span><h2>Project memberships</h2></div></header>{memberships.map((member: any) => <form action={removeManagedMember} className="control-row" key={member.id}><input type="hidden" name="member_id" value={member.id} /><div><strong>{first(member.projects)?.name}</strong><small>{(member.member_roles || []).map((item: any) => first(item.roles)?.name).filter(Boolean).join(", ")}</small></div><button>Remove</button></form>)}</article><article className="control-panel"><header><div><span className="control-eyebrow">ADD ACCESS</span><h2>Place in a project</h2></div></header><form action={addUsersToManagedProject} className="control-profile-add"><input type="hidden" name="user_ids" value={user.id} /><select name="project_id" required defaultValue=""><option value="" disabled>Select project</option>{projects.map((project: any) => <option key={project.id} value={project.id}>{project.name}</option>)}</select><select name="role_name" required defaultValue="Artist">{roles.map((role: any) => <option key={role.name}>{role.name}</option>)}</select><button>Add to Project</button></form></article></section>
